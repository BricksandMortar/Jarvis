    ...
    if org_name is False:
        if path.exists(git_dir):
            ...
            if not isinstance(org_name, str):
                raise ValueError(
                    'No org or username was specified and no existing value could be found from .git directory')
        else:
...
def add_ignores(source):
    with open(out_path + '.gitignore', 'a') as gitignore:
        gitignore.write('#Ignore copied from {}\n'.format(source))
        for ignore in copies:
            gitignore.write(ignore + '\n')
        gitignore.write('#Endignore copied from {}\n'.format(source))

def configure_config():
    replace(out_path + '/_config.yml', 'REPLACE', repo_name)

def setup_install_script():
    replace(out_path + '/script/ciinstall.sh', 'GIT_URL',
            'https://github.com/BricksandMortar/' + repo_name + '/.git')
